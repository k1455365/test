...
class GraphPanel extends Applet implements Runnable {
	...
	synchronized void relax() {
		...
		for (int i = 0; i < nnodes; i++) {
			...
			if (n.road == 0) {
				...
				for (int k = 0; k < 3; k++) {
					if ((n.x < brgleft[k] && n.x > brgright[k])
							&& brgflag[k] == 1) {
						if (temp > brgleft[k] || temp < brgright[k])
							n.x = temp;
					} else if ((n.x < brgleft[k] && n.x > brgright[k])
							&& brgflag[k] == 0)
						if (lghtOrStp == 0)
							brgflag[k] = 1;
						else {
				...
				if ((n.y < brgtop && n.y > brgbottom)
						&& brgflag[n.road - 1] == 1) {
					if (temp > brgtop || temp < brgbottom)
						n.y = temp;
				} else if ((n.y < brgtop && n.y > brgbottom)
						&& brgflag[n.road - 1] == 0)
					if (lghtOrStp == 0)
						brgflag[n.road - 1] = 1;
					else {
	...
	public void paintLghtPeriod(Graphics g) {
		...
		if (lghtOrStp == 1) {
			offgraphics.drawString("Traffic Light Period (1: red, 0: green)",
					600, 50);
			...
			for (int k = 0; k < 3; k++) {
				...
				offgraphics.drawString("Light " + Integer.toString(k + 1),
						temp1 - 40, (temp5 + temp3) / 2 + 5);
				while (temp1 < d.width) {
	...
	public void paintLights(Graphics g) {
		...
		for (int k = 1; k < 4; k++) {
			if (lghtOrStp == 0) {
				...
				g.setColor(Color.black);
				g.fillRect(rdleft[k] - 18, rdtop + 4, lightwidth - 2,
						lightwidth - 2);
	...
	public void paintAxies(Graphics g) {
		...
		for (int k = 0; k < 40; k++) {
			...
			offgraphics.drawRect(k * 3 + temp1, temp2
					- (int) (carpermin[0].carflow[k] * 20 + 1), temp3, temp3);
		}
	}
